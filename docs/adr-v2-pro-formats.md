...
| # | 作業 | なぜ | 状態 |
|---|---|---|---|
| 1 | **実機確認** | 今日 UTI を3つ (`com.adobe.pdf` / docx / xlsx) 宣言した。**既存機能の退行に関わる唯一の未確認事項**で、シミュレータでは自動化できない。とくに **Md の Open In が退行していないか** | **完了** — iPhone (2026-08-07) / **iPad (2026-08-13)。退行なし** |
...
### iPad — **合格 (2026-08-13、iPad / iPadOS 26.6、dev variant の Debug + Metro)**

前回オフラインで残していた唯一の未確認端末。**全項目通過・退行なし。**

| 観点 | 結果 |
|---|---|
| **Md の Open In** (最重要・既存機能) | 退行なし |
| PDF / docx / xlsx / **画像**の 経路A・経路B | すべて到達・表示 |
| **FR-16 ネイティブ表示** | 全画面。互換小窓にならない |
| **読み幅 42em の中央寄せ** | Md・docx とも効いている。**iPad でしか確認できない項目**で、これが初の実地確認 |
| **FR-36 画面の向き** | ホーム含め全画面が4方向。**横向きでも読み幅キャップは外れない**(`max-height: 500px` の判定どおり、背の高い iPad は対象外) |
| xlsx の横スクロール | シートタブ固定のままグリッドのみ横流れ |
| ダークモード | 4形式とも問題なし |

**画像 (FR-45) は iPad 初検証**だった (実装が前回の実機確認より後)。iPhone で踏んだ「測定した箱が iOS のコンテンツインセットを含まず下寄りになる」問題は、`flexGrow` + 既定の stretch に変えた現行実装では**画面比率の違う iPad でも再現しない**ことを確認。
