...
- **ステータス**: **公開済み (2026-08-24)** — 2.0.0 が Apple の審査を通過し App Store で配信開始。**初の有料機能を含むリリース**で、`com.modrift.app.pro` も同時に承認された。以降この ADR は**記録**であり、判断の場ではない。下記「審査へ提出」節。以下は選定時の記録: **PDF・読み込み基盤 (実機検証済み) と docx (シミュレータ確認済み) は採用確定 (Accepted, 2026-08-06)**。**xlsx も実データ検証を経て採用 (2026-08-07)**。**課金も 2026-08-09 に確定 (Accepted): RevenueCat + アカウント無し + プラットフォーム跨ぎの引き継ぎ無し**。SDK のビルドと New Arch 実行時疎通は 2026-08-07 に確認済み。あわせて **v2 には画像閲覧 ([FR-45](Requirements.md#fr-45-画像閲覧-v2--無料)・無料) を追加**した
- **⚠️ `docs/checklist-v2.md` は作らない (2026-08-13 決定)**。MVP〜v1.5 は各リリースに checklist を持つが、**v2 の検証記録はこの ADR に集約する**。実機・iPad・Sandbox の合格記録も、踏んだ罠も既にここに揃っており、checklist を別に起こすと二重管理になって片方が腐る。**v2 の「何を確認したか」を探すときはこの文書を見る**
...
- **電話番号** — 二要素認証で検証。**受信できない番号なら手動確認をリクエストできる**ので 050 でも成立する。候補は下記
...
#### 050 番号の取得 — **候補は購入直前に必ず再確認する**

**個人向けの格安 050 は撤退が相次いでいる**。この ADR を書いている間だけでも SMARTalk (2025-02 終了)・LaLa Call (2026-03-31 新規停止)・G-Call 050 (終了) が消えた。**記憶や過去の記事で選ばないこと** — 実際、この判断の過程で終了済みサービスを3回続けて候補に挙げて外している。**必ず提供元の公式ページで現在の受付状況と料金を見てから決める。**

現時点で確認できている候補 (**2026-08-13 に [公式料金ページ](https://www.subline.jp/price/) で確認**。それ以降は未確認):

| | SUBLINE パーソナル |
|---|---|
| 基本料 | ¥0 |
| 番号利用料 | ¥550 / 番号 / 月 (最大2番号) |
| 開通手数料 | ¥550 / 番号 (初回のみ) |
| 個人契約 | 可 |
| 着信 | 可 (留守番電話・通話録音・拒否あり) |
| 最低利用期間 | 050 は定めなし |

**代替案として携帯番号の副回線 (デュアル SIM) もある**。月額は 050 より高くなりがちだが、番号の存続リスクは低い。**「安いが消えるかもしれない番号」を App Store に公開すること自体がリスク**である点を踏まえて選ぶ — 公開後の電話番号変更が可能かどうかは Apple の公式に記載が無い。
...
### ストア用スクリーンショット (2026-08-13)

iPhone 6.9" / iPad 13" で各6枚を**シミュレータだけで**撮り直した (`store-assets/screenshots/`)。編集画面を残しつつ、旧 `04-settings` を落として PDF / Excel / Word に枠を割いた — **有料機能を売るのに、その画面がストアに1枚も無いのは不利**という理由。

**[撮影レシピ](screenshot-recipe.md)が壊れていたので書き換えた。** iOS 26.4 では `simctl openurl` に**必ず確認ダイアログ**が出るようになり (`modrift-dev://` でも、アプリの起動中・終了中を問わず)、タップ自動化の無いこの環境では deep link 経由の遷移が成立しない。代わりに **`Documents/.shot.json` を読んで起動時に遷移する一時フック**をアプリへ仕込む方式にした。URL 層を通らないのでダイアログが出ず、openurl より確実。

**撮影のために踏んだ罠2つ**:

- **`isPro` を true にするだけでは足りない。** provider の effect を早期 return させると **`isLoading` が永遠に true** のままになり、ビューアが Paywall ではなく「Loading...」で止まる。`isLoading` の初期値も false にする必要があった。**`isLoading` を足した設計がそのまま自分に跳ね返った**形
- **シミュレータでは `materializeFileCoordinated` が解決しない** (Markdown の `readFileCoordinated` は動く)。撮影では `Promise.resolve(fileUri)` に一時置換して回避した。**実機では正常**なので製品側の問題ではないが、シミュレータで v2 ビューアを触るたびに再発する

検体は利用者の実データを使わず生成した (xlsx = SheetJS で書き出し、docx = 最小 OOXML を手組み、PDF = Swift + `CGContext`)。手順はレシピに記録。

### 審査へ提出 (2026-08-13) — **v2 の開発はここで一区切り**

EAS で production ビルド **2.0.0 (12)** を作り App Store Connect へアップロード、掲載文・スクショ12枚・審査メモを転記して、**アプリ本体と `com.modrift.app.pro` を1つの提出としてまとめて審査に出した**。

**提出の入口が事前の想定と違った。** 「バージョンページの『App内課金』セクションに添付する」という手順が世に広く出回っているが、**現行 UI にその欄は無い**。実際は **IAP 側の「審査用に追加」で提出物 (submission) の下書きを作り、そこにバージョンが自動合流する**方式。バージョンを選ぶプルダウンすら無く、「選択したプラットフォームの最新バージョンとともに審査されます」と書かれているだけ。**手順を Apple の公式ヘルプで確認せずに答えて一度外した** — 前回の有料 App 契約のときと同じ失敗で、App Store Connect の UI は解説記事より速く変わる。詳細は [app-store-submission-guide.md](app-store-submission-guide.md#v2-200-固有の手順--初回の-app-内課金を同時提出する)。

**検体ファイルの置き場所**: iOS には既定で PDF / docx / xlsx が無く、審査員が有料機能を確認できないと Guideline 2.1 のリジェクト要因になる。サポートサイトに `https://modrift.github.io/samples/` を立てて審査メモからリンクした。**添付ファイルより URL が主** — 添付は審査員の管理画面側にあり端末へ移す手間がかかるのに対し、URL はテスト端末の Safari で完結する。添付 (PDF 1点) は保険として残した。

**.md は添付しなかった**。審査員は「＋」で新規ノートを作れ、`UIFileSharingEnabled` によりそのノートは**ホームが iCloud でもローカルでも Files に現れる**ので、経路B の確認もそれで足りる。希少な添付枠はアプリ内で作れない形式に使う。

### 公開と本番確認 (2026-08-24) — **完了**

**リジェクト無しで一発通過**し、App Store で公開。**本番の App Store 版で実機確認も済ませた** (Paywall に ¥1,000 が表示され、購入の復元が動く)。Sandbox で通っていても本番の商品取得は別経路なので、ここを見て初めて課金が実在すると言える。

`main` を `v2` へ fast-forward、タグ `v2.0.0`。探索用の `v2-spike` は役目を終えたので削除した (知見はこの ADR に写してある)。**v2 の作業はここで終了**。
