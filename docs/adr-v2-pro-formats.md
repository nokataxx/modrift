...
| # | 作業 | なぜ | 状態 |
|---|---|---|---|
| 1 | **実機確認** | 今日 UTI を3つ (`com.adobe.pdf` / docx / xlsx) 宣言した。**既存機能の退行に関わる唯一の未確認事項**で、シミュレータでは自動化できない。とくに **Md の Open In が退行していないか**。あわせて実際の Open In・大容量 PDF・iPad・ダークモード | 未着手 |
