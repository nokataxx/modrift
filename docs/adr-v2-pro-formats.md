...
2. ~~**バイナリ協調読み込み** (**`v2`**)~~ → **実装済み・シミュレータ確認済み (2026-08-06)。実機検証のみ残**。下記「検証結果」参照
...
- [ ] **プレースホルダの実体化 (本 PoC の本題)**。シミュレータには Google Drive の File Provider が無いため未確認。今日通ったのは「協調ブロックを通してコピーし描画する」経路の成立まで
- [ ] **iCloud ホームの退避ファイル**。素通しさせない設計判断の裏取り
- [ ] PoC 1 の積み残し (実機性能・数十MBの画像主体 PDF) も同じ確認作業に含められる
