...
**位置づけ: 無料コアの完成形（書き味の成熟）。** v1.2 をもって「軽量 Markdown ビューア＆クイックエディタ」を無料で配り切る。その後の v2 で有償の他形式 (PDF/Word/xlsx) 単一ファイル閲覧を載せる ([5.8](#58-収益化--価格モデル))。
...
**位置づけ: v2 (有償 Pro) の前に挟む無料の小規模リリース。** 編集をオプトインに変えて「まずビューア」を製品構造で明確にし、アイコン切り替えでライトなパーソナライズを足す。課金境界 ([5.8](#58-収益化--価格モデル)) は動かさない (Md 編集は無料のまま)。
...
**有償 (買い切り Pro)。** この層が課金境界 ([5.8](#58-収益化--価格モデル))。無料コア (Md 閲覧・編集) は据え置き。Vault ではなく、**Md と同じく「単一ファイルをどこからでも開いて閲覧」**のマルチフォーマット拡張。
...
- **既存の無料機能の有償化** — 一度無料で出した機能 (Md 閲覧・編集等) は無料のまま。課金は v2 以降の新規拡張 (他形式ファイル閲覧) のみに乗せる ([5.8](#58-収益化--価格モデル))
...
- 「意図的に実装しないもの」に明示的な手動Saveボタンを追加 (5.7 参照)
...
- **検索対象は「最近開いたファイル」(≤10件) に限定**。ディスク全体のインデックスは持たない (iOS サンドボックスで不可能かつ不要。件数上限が小さいのでメモリ内検索で足りる)。一般的なファイル検索ではない ([5.7](#57-意図的に実装しないもの) を維持)。
...
> **v1.2 から外す (2026-07-06・改訂16)**: v1.2 の附随機能候補だったが、必要性が薄いため v1.2 では実装せず [5.6 (v3以降: 検討)](#56-v3以降-検討) に送る。判断理由:
...
- **有償 (Pro) 機能**: ここからが買い切り Pro のゲート対象。RevenueCat の `pro` エンタイトルメントで解放する ([5.8](#58-収益化--価格モデル))。無料コア (Md 閲覧・編集) には影響しない。
...
- **対象は Modrift が生成した iCloud コピーに限定**。任意の iCloud ファイルや他フォルダのファイルは対象外 (= 一般的なファイルマネージャにはしない。[5.7](#57-意図的に実装しないもの) を維持)。
...
- 作成先は **Modrift 自身の iCloud コンテナ (`iCloud Drive › Modrift`)** に固定。ここは FR-03 の編集コピー・FR-22 の整理対象と同じ、Modrift がフルアクセスを持つ自分の領域なので、**フォルダピッカー (FR-18)・security scope の問題が一切無い**。任意フォルダに作る一般ファイル作成ではないので [5.7](#57-意図的に実装しないもの) のファイルマネージャ化にも当たらない。
...
- **課金ゲートではない**: Md 編集は無料のまま ([5.8](#58-収益化--価格モデル) の「既存無料機能の有償化はしない」と整合)。ON にすれば従来どおりすべて使える。

**注意 (既存ユーザーの移行)**: 既定 OFF のため、v1.2 から更新したユーザーには「編集ボタンが消えた」と見える。**アップデート後の案内が必須** — (a) 初回起動時のワンタイム案内、(b) 既存ユーザーのみ ON で移行、のどちらにするかは実装時に確定し、リリースノートにも明記する。
...
- デザインの点数・バリエーション内容は実装時に確定。[FR-25](#fr-25-文書スタイルのユーザー選択-プリセット方式-v12--実装済み実機確認済み) の配色プリセット (ネイビー/モノクロ/カラフル) と系統を揃える案を有力とする (選択 UI も FR-25 のカラードットチップと同型にできる)。
...
- RevenueCat (+ StoreKit): 買い切り Pro のエンタイトルメント管理・レシート検証 (5.8)
...
- 買い切り Pro 課金 (RevenueCat + StoreKit、`pro` エンタイトルメントでゲート、5.8)
