...
**位置づけ: 無料コアの完成形（書き味の成熟）。** v1.2 をもって「軽量 Markdown ビューア＆クイックエディタ」を無料で配り切る。次の v2 で有償の他形式 (PDF/Word/xlsx) 単一ファイル閲覧を載せる ([5.7](#57-収益化--価格モデル))。
...
- **既存の無料機能の有償化** — 一度無料で出した機能 (Md 閲覧・編集等) は無料のまま。課金は v2 以降の新規拡張 (他形式ファイル閲覧) のみに乗せる ([5.7](#57-収益化--価格モデル))
...
- **検索対象は「最近開いたファイル」(≤10件) に限定**。ディスク全体のインデックスは持たない (iOS サンドボックスで不可能かつ不要。件数上限が小さいのでメモリ内検索で足りる)。一般的なファイル検索ではない ([5.6](#56-意図的に実装しないもの) を維持)。
...
> **v1.2 から外す (2026-07-06・改訂16)**: v1.2 の附随機能候補だったが、必要性が薄いため v1.2 では実装せず [5.5 (v3以降: 検討)](#55-v3以降-検討) に送る。判断理由:
...
- 作成先は **Modrift 自身の iCloud コンテナ (`iCloud Drive › Modrift`)** に固定。ここは FR-03 の編集コピー・FR-22 の整理対象と同じ、Modrift がフルアクセスを持つ自分の領域なので、**フォルダピッカー (FR-18)・security scope の問題が一切無い**。任意フォルダに作る一般ファイル作成ではないので [5.6](#56-意図的に実装しないもの) のファイルマネージャ化にも当たらない。
...
### Phase 5: v2 (他形式の単一ファイル閲覧 = 有償 Pro) (v1.2 リリース後)
