...
| # | 実データで見えた課題 | 効き目 |
|---|---|---|
| 1 | **列幅 (`!cols`) が反映されず、金額列が画面外に出る** | 大 — 収支計画で数字が見えないのは、色が無いことより痛い |
| 2 | **空行がそのまま描画される** (981行の範囲に中身は 1,521セル。レイアウト用の細い空行が続き、最初の1画面がほぼ空白の格子になる) | 大 |
| 3 | セルの塗り色が落ちる (8色・677セルで区分を表現している) | 中 |
| 4 | `[Red]` 書式の**マイナス値が赤くならない** (`w` は文字列で色を持たない)。収支計画で赤字が赤くない | 中 |
...
- **未確認**: 実機、iPad、ダークモード、数万行規模、`[Red]` 以外の色指定書式。
