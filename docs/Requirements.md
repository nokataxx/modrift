...
- **関連ドキュメント**: `04_DesignDecisions/` 配下のADR (主要な設計判断の経緯と詳細)
...
- **iA Writer は買い切りだが Md 編集に特化しすぎ**: 関連ファイル (PDF、xlsx) を扱えず、Vault 全体の参照には向かない
...
- 編集モード ⇄ プレビューモードのトグル
- **自動保存 (3秒 debounce で Drive へ反映)** ← FR-04
- 保存状態インジケータ (「保存中…」「保存済み」)
- **オフライン編集を許可** ← FR-05
...
### 5.2 v1.1 (Phase 2)
- Share Extension対応 — Drive アプリから「共有」→ Modrift で直接開ける
- ダーク/ライトモード
- フォントサイズ調整
...
- 検討した代替案: 
  (a) 画像表示なし
  (b) HTTPS画像のみ
  (c) フォルダピッカーでローカル画像対応 (フルサポート)
  (d) MVPは(b)、v2で(c)
- 採用: 
  **(d)** — MVPの実装範囲を絞り、早く価値検証する。Document Pickerはアクセス権が選択1ファイルのみのため、ローカル画像は同フォルダ内でもアクセス不可
- v2でフォルダピッカー対応 (FR-18) によりローカル画像表示も可能に
- トレードオフ: 
  MVPでスクリーンショット入りメモは画像が見えない → プレースホルダでファイル名は表示
...
- 検討した代替案: 
  ...
  (c) ハイブリッド (自動+手動)
- 採用: 
  **(b) 自動保存** — iOSメモ・Apple純正アプリと同等のUX、「保存し忘れ」リスクを根本回避、モバイルアプリらしい体験
- トレードオフ: 
  誤編集の取り消しが必要 → v1.1で Undo/Redo (FR-14) で対応。競合リスク → v1.1で警告UI (FR-13) で対応
...
- 検討した代替案: 
  ...
  (c) 許可 + 警告UI
- 採用: 
  **(c) 許可 + 警告UI** (ただしMVPは最小実装) — Obsidian Mobile、iA Writer、Bear、Apple純正メモ全てがオフライン編集を許可、「軽く直す」コンセプトと一致
...
- Drive アプリ等から「共有」シートで Modrift を選択し、ファイルを直接開ける
...
- **同期完了のコールバックがない**:
  `writeAsStringAsync` は「ローカルファイルへの書き込み完了」までしか保証しない。Driveへのアップロード完了はFile Provider内部で進行し、Modriftからは観測できない。UIで「保存しました」とは言えるが、「Driveに反映されました」とは厳密には言えない。MVPでは「保存済み」表示で割り切る
...
- **保存状態の UI 表示**:
  「保存中…」「保存済み」「オフライン (保存待ち)」の状態遷移を明確に。ユーザーが「いま保存されたか」を視覚的に把握できることが重要
...
- [ ] `.md` ファイルを Drive から開ける
- [ ] プレビュー表示が正しい (見出し、リスト、リンク、コード、引用、HTTPS画像)
- [ ] 編集して3秒後に自動保存され、保存状態UI が更新される
