...
- **v1.3 以降**: [FR-28](#fr-28-編集オプトイン-md-編集トグル-v13) の編集オプトインが OFF の間は「＋」を表示しない (編集系導線ごと隠す)。
...
- ローカルを Files に見せるため `UIFileSharingEnabled` / `LSSupportsOpeningDocumentsInPlace` を有効化 (実装時に確認)。

### FR-32: ホームの2ビュー (マイファイル / 最近見た) [v1.4]

ホーム上部にセグメントを置き、**一度に片方**を表示する2ビュー構成にする。

- **マイファイル** (FR-30、場所ビュー): ホームフォルダの中身。ファイル操作 (削除・リネーム・複製・新規・編集) の対象。
- **最近見た** (時間ビュー): 最近開いた**全ファイル**の時系列 (FR-06 の履歴を流用。ホームも Drive/Dropbox も含む)。タップで再オープン。Drive/Dropbox はピッカーの Recents に出ないため、この履歴が再オープンの主手段。
- ＋ (新規、FR-23)・⋮ (3点メニュー、FR-33) は両ビュー共通で上部固定。同一ファイルが両ビューに出るのは Files 同様の2ビュー (混乱要因ではない)。
...
- ホーム = iCloud › Modrift フォルダ (マイファイル) ＋ 最近見た の2ビュー (FR-30/FR-32)
