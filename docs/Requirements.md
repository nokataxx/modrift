...
- **v2**: ライブプレビュー編集 (CodeMirror 一本化) で書き味を飛躍させ、検索・見出しテンプレート等を追加
...
**位置づけ: 無料コアの完成形（書き味の成熟）。** v2 をもって「軽量 Markdown ビューア＆クイックエディタ」を無料で配り切る。次の v3 で有償の他形式 (PDF/Word/xlsx) 単一ファイル閲覧を載せる ([5.7](#57-収益化--価格モデル))。
...
- **Word (.docx) 閲覧**: 本文・見出し・箇条書き・表を整形表示 (docx→HTML 変換の mammoth.js 等。レイアウト完全再現は狙わず「読める整形」に留める。具体ライブラリは v3 着手時に ADR で確定)
...
- 編集履歴 / 過去バージョンへの復元 ([FR-19](#fr-19-編集履歴-v4以降で検討--v2-では見送り)、v2 から送り) — 入れるなら最小の安全網に絞る
...
| 他形式 = PDF / Word (.docx) / xlsx 単一ファイル閲覧 (v3、[FR-21〜](#fr-21-関連ファイル対応-v3--pro-概略のみ)) | **有償 (Pro)** |
...
### FR-19: 編集履歴 [v4以降で検討 / v2 では見送り]

> **v2 から外す (2026-07-06・改訂16)**: v2 の附随機能候補だったが、必要性が薄いため v2 では実装せず [5.5 (v4以降: 検討)](#55-v4以降-検討) に送る。判断理由:
...
> **経緯**: 当初のフォルダ Vault 内に作る版は改訂11で廃止 (作成先の FR-18 フォルダピッカーが iOS 制約で成立しないため)。その際「iCloud › Modrift に作る縮小形なら再検討の余地あり」と残し ([1章](#1-概要))、**改訂19 (2026-07-06) で縮小形として v2 に復活**。作成先を **常に iCloud Drive › Modrift フォルダ (Modrift 自身の ubiquity container)** に固定することで、フォルダピッカー依存を完全に外した。
...
### FR-25: 文書スタイルのユーザー選択 (プリセット方式) [v2 — 実装済み・実機確認済み]
...
- **v2 から送り (改訂16)**: 編集履歴 (FR-19) → v4以降で検討。Undo/競合検知で大半カバー・自動保存と相性が悪い・軽量エディタの範疇を超えるため
- 新規 Md ノート作成 (FR-23、iCloud › Modrift 固定の縮小形) — 改訂19 で v2 に復活
