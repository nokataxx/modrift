...
| **Modrift** | **Vault参照+軽編集** | ◎ Drive直接 | ◎ v3で対応 | ◎ 軽快 | Md/Vaultは無料・関連ファイルは買い切りPro |
...
**位置づけ: 無料コアの完成形（Vault 体験の成熟）。** v2 をもって「軽量 Md / Vault クライアント」を無料で配り切る。次の v3 で有償の関連ファイル対応を載せる ([5.7](#57-収益化--価格モデル))。

**v2 の三本柱（主役）:**

- **ライブプレビュー編集 — EnrichedMarkdownTextInput 移行 (Obsidian 形式)** ← [FR-20](#fr-20-enrichedmarkdowntextinput-への移行-v2)。書き味の飛躍。
- **Vault ブラウザ — 指定フォルダを Modrift 独自 UI で参照、日常導線から Document Picker を外す** ← [FR-24](#fr-24-vault-ブラウザ-v2)。毎日の導線が変わる。
- **ローカル画像表示 — フォルダピッカー対応** ← [FR-18](#fr-18-フォルダピッカー対応-ローカル画像表示-v2)。「画像入りノートが読める」。

**付随機能（おまけ）:**

- 全文検索 (最近開いたファイル内) ← FR-15
- 新規 Md ファイルの作成 (Vault フォルダ内) ← FR-23
- 見出しスタイルのユーザー選択 (テンプレート) ← FR-25
- 編集履歴 (アプリ内ローカル) ← FR-19
- Obsidian風 `![[...]]` リンク (同フォルダ内、ピッカー経由) ← FR-17
- iPad 大画面レイアウト最適化 ← FR-16 (左右分割は廃止)

### 5.4 v3: 関連ファイル対応 (Phase 5)

**有償 (買い切り Pro)。** この層が課金境界 ([5.7](#57-収益化--価格モデル))。無料コア (Md/Vault/画像) は据え置き。
...
- **既存の無料機能の有償化** — 一度無料で出した機能 (Md 編集・Vault・ローカル画像等) は無料のまま。課金は v3 以降の新規拡張 (関連ファイル) のみに乗せる ([5.7](#57-収益化--価格モデル))

### 5.7 収益化 / 価格モデル

Modrift は**無料コア + 有償拡張**の二層。製品の段階的ビジョン (MVP→Vault→関連ファイル) の**後半 (関連ファイル) を有償**にする。

**無料 / 有償の境界 (確定):**

| 範囲 | 区分 |
|---|---|
| Markdown 編集・Vault・ローカル画像 (MVP〜v2) | **無料** |
| 関連ファイル = PDF / xlsx 閲覧 (v3、[FR-21〜](#fr-21-関連ファイル対応-v3--pro-概略のみ)) | **有償 (Pro)** |
...
- **有償 (Pro) 機能**: ここからが買い切り Pro のゲート対象。RevenueCat の `pro` エンタイトルメントで解放する ([5.7](#57-収益化--価格モデル))。無料コア (Md/Vault/画像) には影響しない。
...
### Phase 4: v2 (無料コアの完成 = Vault 体験の成熟) (公開後、ユーザーフィードバック次第)

- **三本柱**: ライブプレビュー編集 (FR-20)、Vault ブラウザ (FR-24)、ローカル画像表示 (FR-18)
- 付随: 検索、新規ファイル作成、見出しテンプレート、編集履歴、Obsidian風リンク、iPad 大画面最適化 (左右分割は廃止)

### Phase 5: v3 (関連ファイル対応 = 有償 Pro) (v2 リリース後)
