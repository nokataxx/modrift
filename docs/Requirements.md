...
- **① 履歴のロケーション行の冗長性 → 採用 (実装済み)**: 「最近見た」(→「履歴」) のうち**ホーム内ファイルの2行目を「更新日時 · ホーム」に変更**。ロケーション行を空にすると行の高さが不揃いになるため、空欄化ではなく**更新日時＋短ラベルに差し替える**。更新日時はフォーカス時に home ファイルのみ `File.modificationTime` を stat (退避・移動で読めなければ日付を省きラベルのみ)。**ホーム外 (Drive/Dropbox/iCloud Drive 等) は従来どおりロケーション表示のまま** — 出自の手がかり (FR-06) を残す。
...
- **④ ホームへコピー時、履歴を外部原本からホームのコピーへ付け替え → 採用**: 外部ファイル (例: Google Drive) をピッカーで開くと「履歴」に記録され、「ホームにコピー」するとコピーが「ホーム」に載るため、**同じ内容が2箇所に見えて紛らわしい**。**コピーした場合のみ**、`performCopyToHome` でコピー成功後に**コピー (ホームファイル) を履歴に記録**し、続けて**元の外部ファイルの履歴エントリを削除**する (コピーしなかった場合は元の履歴をそのまま残す)。結果、履歴は外部原本ではなく**編集可能なホームのコピーを指す** (①により「更新日時 · ホーム」表示)。**これは [FR-34](#fr-34-ホームにコピー-view-only-ファイルの明示コピー-v14) の当初方針「元ファイルは最近見たに残す・コピーは履歴に載せない」(decision ⑤) を反転**する。
...
**表示**: 編集モードでソフトウェアキーボード表示中のみ、**キーボード直上に固定**した横スクロールのアイコン列。プレビュー時・キーボード非表示時は出さない。ホーム限定編集 (ポリシーA) と整合し、編集可能なファイルでのみ現れる。
