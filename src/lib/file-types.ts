 ...
 */
export const SUPPORTED_EXTENSIONS = ['.md', '.markdown', '.txt', '.text', '.pdf'] as const;
 ...
 */
export function routeForFile(name: string): '/viewer' | '/pdf-viewer' {
  return name.toLowerCase().endsWith('.pdf') ? '/pdf-viewer' : '/viewer';
}
